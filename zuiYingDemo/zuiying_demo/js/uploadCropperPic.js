$(document).ready(function () {
    ...
    $(function () {
        ...
        //视频上传
        var videoAdd = $('[name="video_file"]');
        var videoAddMore = $('[name="video_add_more"]');
        videoAdd.change(function () {
            videoChange('video_file')
        });
        videoAddMore.change(function () {
            videoChange('video_add_more')
        });
        function videoChange(fileName) {
            var videoInf = $('[name="' + fileName + '"]').prop('files')[0];
            var videoName = videoInf['name'];
            var videoSize = videoInf['size'];
            console.log(videoInf, videoName, videoSize);

            var index = videoName.indexOf(".");
            var videoNameType = videoName.substring(index);
            // console.log(videoNameType);
            $('.video-add-item-name').text(videoName);

            if (videoNameType != ('".mp4"||".rmvb"||".avi"||".ts"')) {
                alert("视频上传格式不正确，请重新选择");
                $('[name="video_file"]').value = "";
            } else if (videoSize > 1024 * 1024 * 1024) {
                alert("视频大于1G，请压缩后重新上传");
                $('[name="video_file"]').value = "";
            } else {
                $('.set-card-box').hide();
                $('.set-collection-box').hide();
